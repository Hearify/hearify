import { useTranslation } from 'react-i18next';

import styles from '@src/components/Header/Header.module.scss';
import logoIcon from '@src/assets/images/logo.svg';

import type { BrandKitInterface } from '@src/components/BrandKit/BrandKit';

interface HeaderPreviewProps {
  brandKit: BrandKitInterface;
}

const HeaderPreview = ({ brandKit }: HeaderPreviewProps) => {
  const { t } = useTranslation('general');

  return (
    <header className={styles.header}>
      <div className={styles.logoContainer}>
        <img src={brandKit.logoUrl ? brandKit.logoUrl : logoIcon} alt="Logo Image" />
      </div>

      <button className={styles.logout} style={{ color: brandKit.buttonFill, borderColor: brandKit.buttonFill }}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M14 8V6C14 5.46957 13.7893 4.96086 13.4142 4.58579C13.0391 4.21071 12.5304 4 12 4H5C4.46957 4 3.96086 4.21071 3.58579 4.58579C3.21071 4.96086 3 5.46957 3 6V18C3 18.5304 3.21071 19.0391 3.58579 19.4142C3.96086 19.7893 4.46957 20 5 20H12C12.5304 20 13.0391 19.7893 13.4142 19.4142C13.7893 19.0391 14 18.5304 14 18V16"
            stroke={brandKit.buttonFill}
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M7 12H21M21 12L18 9M21 12L18 15"
            stroke={brandKit.buttonFill}
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>
        <span style={{ fontFamily: brandKit.font?.family && brandKit.font.family }}>{t('exit')}</span>
      </button>
    </header>
  );
};

export default HeaderPreview;
